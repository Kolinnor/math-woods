...
const db=write('db.js',`function matches(row,where){if(!where)return true;return Object.entries(where).every(([key,value])=>key==='AND'?value.every(w=>matches(row,w)):key==='OR'?value.some(w=>matches(row,w)):key==='translations'?row.translations.some(t=>matches(t,value.some)):value&&typeof value==='object'&&'in'in value?value.in.includes(row[key]):row[key]===value)};export const prisma={mathematician:{findMany:async({where})=>window.people.filter(row=>matches(row,where))},libraryEra:{findMany:async()=>[['ancient',-3500,'Antiquité'],['medieval',500,'Moyen Âge'],['early-modern',1500,'Époque moderne'],['modern',1800,'XIXe siècle'],['contemporary',1950,'Époque contemporaine']].map(([slug,startYear,name])=>({slug,startYear,color:'#6d6555',nameFr:name,nameEn:name,descriptionFr:'',descriptionEn:''}))}};`);
...
try {for(const locale of ['fr','en']){
  ...
  // On a phone the filters sit behind the "Filtres" toggle.
  if(!(await page.locator('input[name=review]').isVisible()))await page.locator('.library-filter-toggle').click();
