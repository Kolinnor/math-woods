...
if (process.env.NODE_ENV === "production" && process.env.ALLOW_DEMO_SEED !== "true") {
  throw new Error("Refusing to run demo seed data in production. Set ALLOW_DEMO_SEED=true only for an intentional restore/dev task.");
}
...
async function main() {
  ...
  await prisma.problemScore.upsert({
    where: { userId_problemId: { userId: curator.id, problemId: problem.id } },
    update: { naturality: 24 },
    create: { userId: curator.id, problemId: problem.id, naturality: 24 }
  });
