...
export async function createConceptAction(formData: FormData) {
  ...
  if (!title || !bodyMarkdown) throw new Error("Title and content are required.");
...
export async function updateConceptAction(conceptId: number, formData: FormData) {
  ...
  const aliases = parseAliases(formData.get("aliases"));
  const references = parseReferences(formData.get("references"));
  const editSummary = String(formData.get("editSummary") ?? "").trim() || "Concept edited";
  const requestedStatus = String(formData.get("status") ?? "STUB") as ConceptStatus;
  const status =
    (user.role === "MODERATOR" || user.role === "ADMIN") && Object.values(ConceptStatus).includes(requestedStatus)
      ? requestedStatus
      : undefined;
