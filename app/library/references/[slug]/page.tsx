...
export default async function LibraryReferencePage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ duplicate?: string; lang?: string }> }) {
  ...
  return (
    <ForestPageLayout className="library-reference-detail-page" title={<>{translation?.displayTitle ?? entry.canonicalTitle}{translation && <ContentLanguageFallback language={translation.language} expectedLanguage={contentLanguage} />}</>} description={referenceTypeLabel(entry.referenceType, locale)} heroImage="/art/oak-grove.jpg">
      ...
      <LibraryEntryToolbar locale={locale} backHref="/library/references" backLabel={locale === "fr" ? "Retour aux références" : "Back to references"} href={`/library/references/${entry.slug}`} languages={entry.translations.map(t => t.language)} activeLanguage={editLanguage} status={entry.status} reviewed={Boolean(entry.reviewedAt)} />
