...
import { canArchiveLibraryEntry, canEditLibraryDraft, canReviewLibraryEntry, hasTrustedPrivileges } from "@/lib/permissions";
...
export default async function EditHistoryMilestonePage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ lang?: string }> }) {
  ...
  return <ForestPageLayout className="library-editor-page" titleBelowHero title={locale === "fr" ? "Modifier le repère" : "Edit milestone"} meta={<p>{translation?.title ?? entry.slug}</p>} heroImage="/art/brook-in-the-forest.jpg"><div className="library-editor-toolbar"><LibraryEditorBack href={`/library/history/${entry.slug}?lang=${contentLanguage}`} locale={locale} />{hasTrustedPrivileges(user.role) && entry.status === "PUBLISHED" && <LibraryTimelineToggle entity="milestone" id={entry.id} featured={entry.featuredOnTimeline} locale={locale} />}<LibraryReviewActions entity="milestone" id={entry.id} locale={locale} status={entry.status} needsReviewAfterEdit={!entry.reviewedAt} baseUpdatedAt={entry.updatedAt.toISOString()} canReview={canReviewLibraryEntry(user, entry)} canArchive={canArchiveLibraryEntry(user)} compact /></div><LibraryTranslationEditorNav baseHref={`/library/history/${entry.slug}/edit`} locale={locale} activeLanguage={contentLanguage} existingLanguages={entry.translations.map((item) => item.language)} /><HistoryMilestoneForm action={saveHistoryMilestoneFormAction.bind(null, entry.id)} locale={locale} contentLanguage={contentLanguage} baseUpdatedAt={entry.updatedAt.toISOString()} options={options} values={{ ...entry, translation, mathematicianIds: entry.mathematicians.map((item) => item.mathematicianId), referenceIds: entry.referenceLinks.map((item) => item.referenceId), conceptIds: entry.conceptLinks.map((item) => item.conceptId) }} /></ForestPageLayout>;
}
