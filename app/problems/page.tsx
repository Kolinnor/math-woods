...
function advancedFilterWhere(filter: ProblemFilterRow): Prisma.ProblemWhereInput | null {
  ...
  if (filter.field === "domain") {
    ...
    return domainFilter ? { domain: domainFilter } : null;
  }
...
export default async function ProblemsPage({
  ...
  const advancedClauses = advancedFilters
    .map((filter) => advancedFilterWhere(filter))
  ...
  const whereClauses: Prisma.ProblemWhereInput[] = [
    ...
    ...(query
      ...
      : []),
    ...(tagSlug ? [{ tags: { some: { tag: { slug: tagSlug } } } } satisfies Prisma.ProblemWhereInput] : []),
  ...
  const [totalProblems, tags] = await Promise.all([
    ...
    <div className="directory-page">
      <div className="page-header">
        <div>
          ...
          <p className="muted mt-1">Browse, attempt, annotate, then discuss after a delay.</p>
        </div>
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="meta">
                  @{problem.author.username} / {problem._count.attempts} attempts
                </p>
                <p className="meta">
                  {domainLabel(problem.domain)} / {qualityLabel(problem.qualityStatus)} /{" "}
                  {problem._count.favorites} favorites
                </p>
