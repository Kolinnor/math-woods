...
async function relatedQuotesForConcept(conceptId: number) {
  try {
    return await prisma.quoteConcept.findMany({
      where: { conceptId },
      include: { quote: true },
...
export default async function ConceptPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [outgoingLinks, backlinks, relatedQuotes, watched] = await Promise.all([
    ...
    relatedQuotesForConcept(concept.id),
    user
      ...
      <article>
        <div className="reading-header mb-5">
          ...
          <p className="muted mt-1">
            ...
            {concept.lastEditedBy ? ` · edited by @${concept.lastEditedBy.username}` : ""}
          </p>
      ...
      <aside className="grid content-start gap-5">
        ...
        <section className="sidebar-section">
          ...
          <div className="grid gap-2 text-sm">
            ...
            {relatedQuotes.length === 0 && <p className="muted">No related quotes.</p>}
          </div>
        </section>
