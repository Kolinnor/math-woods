...
export function ReferenceForm({ action, locale, contentLanguage = locale, baseUpdatedAt, values = {} }: { action: (state: { error: string }, data: FormData) => Promise<{ error: string }>; locale: "en" | "fr"; contentLanguage?: "en" | "fr"; baseUpdatedAt?: string; values?: ReferenceFormValues }) {
  ...
  return (
    <form onSubmit={event => {
      ...
      <div className="library-editor-field"><span>{fr ? "Présentation" : "Description"}</span><MarkdownEditor name="descriptionMarkdown" ariaLabel={fr ? "Présentation" : "Description"} initialValue={values.translation?.descriptionMarkdown ?? ""} minHeight="14rem" /></div>
      </div></details>
